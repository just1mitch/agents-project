...
class Mario:
    def __init__(self, state_dim, action_dim, save_dir, checkpoint=None):
        ...
        self.sync_every = 1.2e4  # Frequency (in steps) at which the target Q-network is updated with the online Q-network's weights. [1e4 - 2e4]
    ...
    def cache(self, state, next_state, action, reward, done):
        ...
        # Agent may not work if a GPU is not available.

    def recall(self):
        """Retrieve a batch of experiences from memory."""
        # Sample experiences from memory
        # This is a uniform random sample of previous experiences based on the batch size
...
# The original source is used to calculate the TD target, but the TD estimate is calculated using the online network.
    ...
    def save(self, episode):
        ...
        torch.save({
         ...
         #   'memory': list(self.memory)[-10000:] - We can save the last 10000 experiences, which is better than starting from scratch with a burnin
         # However this makes the checkpoint files very large, so it is not used.
        }, save_path)
    ...
    def load(self, load_path):
        ...
        #self.memory.extend(loaded_memory) - Not used, see above
